# GBFR Magic

Granblue Fantasy: Relink 内存修改工具 (Rust + Tauri 2)

**语言:** [English](README.md) · [中文](README_ZH.md) · [日本語](README_JA.md)

## 功能

### 队伍编辑器
- 直接修改 5 个队伍槽位的角色 ID（`exe+0x701C420`，每槽 0x10）

### 露莉亚切换
- 一键切换露莉亚 / 恢复为卡塔莉娜（合法角色）
- 露莉亚战斗/回城/城镇移动正常，但**打开菜单会闪退**（敌人模型数据）
- 流程：切换露莉亚 → 进战斗 → 开菜单/存档前恢复为卡塔莉娜（防坏档）

### 角色修改（罗兰因子方法）
- 让因子界面显示任意角色（如罗兰），从而能打开装备/因子窗口
- 原理：定位"当前选中角色 ID"指针，改成目标角色 ID
- 步骤：因子界面选中角色A → 扫描 → 切到角色B → 过滤 → 切回A → 过滤 → 写入目标

### 战斗增强
- **无CD**：技能无冷却
- **无限血**：受到伤害清零，自己打怪正常
- 可随时开关，关闭时还原原始字节

### 角色速查
- 全部角色 ID 参考（十六进制 + 十进制）

### 连接控制
- 手动"连接 / 断开"按钮（不自动连接）
- 连接时验证游戏进程和队伍指针

### i18n
- 中文 / 日本語 / English 界面，顶栏切换，持久化

## 特殊角色状态

| 角色 | 入队 | 战斗 | 菜单 | 因子 |
|---|---|---|---|---|
| 罗兰 | ✅ | ✅ | ✅ | ✅ 全功能 |
| 露莉亚 | ✅ | ✅ | ❌ 闪退 | ❌ |
| 龙人伊德 | ✅ | ❌ 非战斗角色 | - | ❌ |

## 构建

```bash
cd src-tauri
cargo build --release
# 输出: target/release/gbfr_tool.exe
```

## 使用

1. 启动游戏
2. 运行 `gbfr_tool.exe`
3. 点 **连接** 按钮
4. 使用各标签页功能

## 技术

- Rust + Tauri 2（windows-sys FFI）
- 队伍指针：`exe+0x701C420`（5 槽位 x 0x10）
- 战斗增强通过运行时 patch（RVA 固定，重启后重新应用）
- 角色修改通过内存扫描 + 过滤定位选中角色指针（每次重启需重新定位）
